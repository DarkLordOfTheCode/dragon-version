evolutions = {
    # Custom Draconia lines
    "Frodger":    ("Frogandi",   16),
    "Frogandi":   ("Frogainian", 36),
    "Buliz":      ("Bulits",     16),
    "Bulits":     ("Bulitzer",   36),
    "Falake":     ("Faelke",     16),
    "Faelke":     ("Psyake",     36),
    "Flamlet":    ("Serfling",   18),
    "Serfling":   ("Serfyre",    35),
    "Frostlet":   ("Glacilet",   18),
    "Glacilet":   ("Glaciwyrm",  35),
    "Riplet":     ("Aqualet",    18),
    "Aqualet":    ("Aqualisk",   35),
    "Sparklet":   ("Voltlet",    18),
    "Voltlet":    ("Voltscale",  35),
    "Ironlet":    ("Steelbit",   18),
    "Steelbit":   ("Ironmaw",    35),
    "Misting":    ("Phantling",  18),
    "Phantling":  ("Phantail",   35),
    # Gen 1
    "Dratini":    ("Dragonair",  30),
    "Dragonair":  ("Dragonite",  55),
    "Roggenrola": ("Boldore",    25),
    "Boldore":    ("Gigalith",   40),
    "Clefairy":   ("Clefable",   36),
    "Snubbull":   ("Granbull",   23),
    # Gen 2
    "Horsea":     ("Seadra",     32),
    # Gen 3
    "Bagon":      ("Shelgon",    30),
    "Shelgon":    ("Salamence",  50),
    "Swablu":     ("Altaria",    35),
    "Trapinch":   ("Vibrava",    35),
    "Vibrava":    ("Flygon",     45),
    # Gen 4
    "Gible":      ("Gabite",     24),
    "Gabite":     ("Garchomp",   48),
    # Gen 5
    "Axew":       ("Fraxure",    38),
    "Fraxure":    ("Haxorus",    48),
    "Deino":      ("Zweilous",   50),
    "Zweilous":   ("Hydreigon",  64),
    # Gen 6
    "Goomy":      ("Sliggoo",    40),
    "Sliggoo":    ("Goodra",     50),
    "Noibat":     ("Noivern",    48),
    "Tyrunt":     ("Tyrantrum",  39),
    # Gen 7
    "Jangmo-o":   ("Hakamo-o",   35),
    "Hakamo-o":   ("Kommo-o",    45),
    # Gen 1 additions
    "Magikarp":   ("Gyarados",   20),
    "Scyther":    ("Scizor",     38),
    "Gastly":     ("Haunter",    25),
    "Haunter":    ("Gengar",     40),
    "Onix":       ("Steelix",    40),
    # Gen 4 additions
    "Cranidos":   ("Rampardos",  30),
    "Honedge":    ("Doublade",   35),
    "Doublade":   ("Aegislash",  50),
    "Rookidee":   ("Corvisquire",18),
    "Corvisquire":("Corviknight",38),
    # Gen 2 additions
    "Misdreavus": ("Mismagius",  42),
    "Zorua":      ("Zoroark",    30),
    "Sandile":    ("Krokorok",   29),
    "Krokorok":   ("Krookodile", 40),
    "Pawniard":   ("Bisharp",    52),
    "Bisharp":    ("Kingambit",  65),
    "Litwick":    ("Lampent",    41),
    "Lampent":    ("Chandelure", 55),
    "Sneasel":    ("Weavile",    40),
    "Houndour":   ("Houndoom",   24),
    "Gligar":     ("Gliscor",    40),
    "Larvitar":   ("Pupitar",    30),
    "Pupitar":    ("Tyranitar",  55),
    # Gen 3 additions
    "Beldum":     ("Metang",     20),
    "Metang":     ("Metagross",  45),
    "Aron":       ("Lairon",     32),
    "Lairon":     ("Aggron",     42),
    # Gen 7 additions
    "Salandit":   ("Salazzle",   33),
    "Toxel":      ("Toxtricity", 30),
    # Gen 8
    "Dreepy":     ("Drakloak",   50),
    "Drakloak":   ("Dragapult",  60),
    # Gen 9
    "Frigibax":   ("Arctibax",   35),
    "Arctibax":   ("Baxcalibur", 54),
    # Psychic types
    "Abra":       ("Kadabra",    16),
    "Kadabra":    ("Alakazam",   36),
    "Ralts":      ("Kirlia",     20),
    "Kirlia":     ("Gallade",    30),
    "Bronzor":    ("Bronzong",   33),
    # Aziza's line
    "Poliwag":    ("Poliwhirl",  15),
}

# (pokemon_name, item_name) -> evolved_form
item_evolutions = {
    ("Seadra",    "Dragon Stone"): "Kingdra",
    ("Applin",    "Dragon Stone"): "Flapple",
    ("Poliwhirl", "Water Stone"):  "Poliwrath",
}
